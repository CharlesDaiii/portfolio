...
import { u as useInViewport, c as cssProps, a as classes, n as numToMs, r as resolveSrcFromSrcSet, M as ModelAnimationType } from "./server-build-CQcGY3tM.js";
import { useState, useRef, useEffect, useCallback, createRef, startTransition } from "react";
import { WebGLRenderer, SRGBColorSpace, PerspectiveCamera, Scene, Group, AmbientLight, DirectionalLight, WebGLRenderTarget, PlaneGeometry, MeshBasicMaterial, Mesh, OrthographicCamera, MeshDepthMaterial, ShaderMaterial, Color, Vector3, MathUtils, VideoTexture, LinearFilter } from "three";
import { HorizontalBlurShader, VerticalBlurShader } from "three-stdlib";
import { r as removeLights, c as cleanScene, a as cleanRenderer, m as modelLoader, t as textureLoader } from "./three-Bpxpz58r.js";
...
const model = "_model_1j475_2";
const canvas = "_canvas_1j475_11";
const styles = {
  model,
  canvas
};
...
const Device = ({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const { texture, position, url, type } = model2;
      let loadFullResTexture;
      let playAnimation;
      try {
        if ((texture == null ? void 0 : texture.type) === "video") {
        ...
        gltf.scene.traverse(async (node) => {
          if (node.material) {
            if (type === "quest3") {
              node.material.color = new Color(8421504);
            } else {
        ...
        if (model2.animation === ModelAnimationType.Quest3Rotate) {
          playAnimation = () => {
